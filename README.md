...
requires a login. Browsers get the native HTTP Basic Auth prompt (a
username/password dialog). Scripts and the API demo authenticate by sending a
preemptive `Authorization: Basic user:pass` header (e.g. `curl -u user:pass`).
If `CREDENTIALS` is unset or malformed, the app logs a warning and runs with
**no gate at all**, so double-check the format.

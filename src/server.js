...
// Optional global gate. If CREDENTIALS is set, require HTTP Basic Auth on every
// request - this keeps the whole app private, including the /api/* routes below
// that operate on the owner's storage with no auth of their own (browse/upload/
// download/delete). Without it, anyone who finds the deployed URL can read and
// delete the owner's files.
//
// This uses the browser's native Basic Auth prompt (a 401 + WWW-Authenticate
// challenge). A preemptive `Authorization: Basic user:pass` header is accepted
// too, so curl/scripts and the API demo page authenticate without a dialog.
//
...
if (CREDS) {
  app.use((req, res, next) => {
    ...
    if (hasValidBasic(req)) return next();
